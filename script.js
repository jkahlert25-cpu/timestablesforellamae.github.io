...
const quizQCurrentEl     = document.getElementById('quiz-q-current');
...
function handleKeydown(e) {
  ...
  if (practiceScreen.classList.contains('active') && !submitBtn.disabled) {
    checkAnswer();
  }

  if (quizScreen.classList.contains('active') && !quizSubmitBtn.disabled) {
    quizCheckAnswer();
  }
}
...
function startQuiz() {
  ...
  quizState.questionsLeft  = 12;
  quizState.timeLeft       = 30;
  quizState.finished       = false;
  quizState.awaitingNext   = false;

  quizTimerEl.textContent  = '30';
  quizTimerEl.style.color  = '';
  quizQCurrentEl.textContent = '1';

  clearQuizFeedback();
  showScreen(quizScreen, homeScreen);
  quizNewQuestion();
  quizAnswerInput.focus();

  quizState.timerInterval = setInterval(() => {
    quizState.timeLeft--;
    quizTimerEl.textContent = quizState.timeLeft;

    if (quizState.timeLeft <= 10) {
      quizTimerEl.style.color = 'var(--clr-wrong)';
    }
...
function quizNewQuestion() {
  ...
  quizState.currentA       = a;
  quizState.currentB       = b;
  quizState.correctAnswer  = a * b;
  quizState.awaitingNext   = false;

  const questionNumber = 13 - quizState.questionsLeft;
  quizQCurrentEl.textContent = questionNumber;

  quizQuestionText.textContent = `${b} \u00d7 ${a} = ?`;
  triggerBounce(quizQuestionCard);
...
function quizCheckAnswer() {
  if (quizState.finished || quizState.awaitingNext) return;
  ...
  if (userAnswer === quizState.correctAnswer) {
    ...
    showQuizFeedback('correct', `Correct! ${quizState.currentB} \u00d7 ${quizState.currentA} = ${quizState.correctAnswer}`);
    ...
    triggerShake(quizQuestionCard);
    quizAnswerInput.value = '';
    quizAnswerInput.focus();
    showQuizFeedback('wrong', 'Incorrect \u2014 try again.');
  }
}
...
function quizComplete() {
  ...
  showQuizFeedback('correct', 'Good Job!');
}
...
function quizTimeUp() {
  ...
  showQuizFeedback('wrong', 'Try Again!');
}

function showQuizFeedback(type, message) {
  quizFeedbackMsg.textContent = message;
  quizFeedbackMsg.className = `feedback-msg ${type}`;
}

function clearQuizFeedback() {
  quizFeedbackMsg.textContent = '';
  quizFeedbackMsg.className = 'feedback-msg';
}
...
function quitQuiz() {
  ...
  showScreen(homeScreen, quizScreen);
...
function attachEventListeners() {
  ...
  quizSubmitBtn.addEventListener('click', quizCheckAnswer);
