...
const state = {
  ...
  // Whether the current question has been answered yet
...
function checkAnswer() {
  // Prevent checking twice on the same question
  ...
  // Parse the answer — use parseInt to be strict about numbers
  const userAnswer = parseInt(raw, 10);

  // Handle non-numeric or weird input
  if (isNaN(userAnswer)) {
    showFeedback('wrong', '🤔 That doesn\'t look like a number. Try again!');
    answerInput.value = '';
    answerInput.focus();
    return;
  }

  // Mark as answered so Enter/Submit can't fire twice
  state.answered = true;
  ...
  if (userAnswer === state.correctAnswer) {
    ...
    showFeedback('correct', `${praise} ${state.currentA} × ${state.currentB} = ${state.correctAnswer} 🎉`);
    ...
    showFeedback(
      'wrong',
      `Not quite! ${state.currentA} × ${state.currentB} = ${state.correctAnswer}. Keep going! 💪`
    );

    // Shake the question card for visual feedback
...
/** Array of encouraging praise phrases for correct answers */
